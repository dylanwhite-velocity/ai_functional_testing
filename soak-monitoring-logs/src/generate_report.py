...
def summarize_pod_logs(data: Dict[str, Any]) -> str:
    ...
    lines.append(f"Total items checked: {summary.get('total_items_checked', 0)}")
    lines.append(f"Items with errors: {summary.get('items_processed', 0)}")
    ...
    if rated_envs:
        ...
        for env in rated_envs:
            ...
            lines.append(f"    Total items checked: {env.get('total_items_checked', 0)}")
